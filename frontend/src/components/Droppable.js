 ...
 */
export default function Droppable({ acceptItemTypes, dropFunc, className, canDropClassName, isOverClassName, children }) {
    ...
    return (
        <div ref={dropRef} className={`${className} ${canDrop && canDropClassName} ${isOver && isOverClassName}`}>
            {children}
        </div>
    )
}